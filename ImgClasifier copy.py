...
path = 'D:\\DocumentosD\\Python\\Vision-artificial\\Imagenes\\TrainF' #Ruta donde se encuentran las imagenes de referencia
sift = cv.SIFT_create(nfeatures=1500) #función utilizada para crear un objeto sift.
...
for cl in myList: #Para todos los archivos dentro de la ruta se guardan como imagen en escala de grises
    imgCur = cv.imread(f'{path}\\{cl}') #funcion para leer la imagen en escala de grises
...
cap = cv.VideoCapture('D:\\DocumentosD\\Python\\Vision-artificial\\Video\\prueba\\Tijeras hold5s.mp4')
